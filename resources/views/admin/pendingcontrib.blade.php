@if(session('contrib_approve'))
  <div id="alert-3" class="flex max-w-screen-xl p-6 my-16 mx-auto items-center p-4 mb-4 text-blue-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
    <svg class="flex-shrink-0 w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
      <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"/>
    </svg>
    <span class="sr-only">Info</span>
    <div class="ms-3 font-bold">
    {{ session('contrib_approve') }}
    </div>
    <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-green-50 text-green-500 rounded-lg focus:ring-2 focus:ring-blue-400 p-1.5 hover:bg-green-200 inline-flex items-center justify-center h-8 w-8 dark:bg-gray-800 dark:text-blue-400 dark:hover:bg-gray-700" data-dismiss-target="#alert-3" aria-label="Close">
...
<div class="container max-w-screen-xl mx-auto card p-4 pb-8 rounded-lg">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl dark:text-gray-50 font-bold">Pending Contributions</h2>
    </div>
    <div class="relative sm:overflow-none overflow-auto">
        <table class="w-full text-left rtl:text-right text-gray-300 dark:text-gray-300">
            ...
            <tbody>
            @foreach($pending_contribs as $contrib)
                @php
                    $encryptedid_contrib = encryptid($contrib['contribution_id']);
                @endphp
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    <td class="px-6 py-4">
                        <button onclick="showReceipt('<?php if($contrib['receipt_photo']) echo asset($contrib['receipt_photo']); else echo asset('/public/storage/receiptimg/artworks-noreceipt.jpg'); ?>');" class="inline-flex gap-1 border dark:border-gray-800 dark:bg-gray-700 hover:dark:bg-gray-600 rounded-lg px-2 py-1">
                    ...
                    <th scope="row" class="px-6 py-4 font-medium whitespace-nowrap">
                        <div class="flex gap-1 items-center justify-start">
                            <img class="w-10 h-10 rounded-full sm:inline hidden" src="{{ asset($contrib['image']) }}" alt="Jese image">
                    ...
                    <td class="px-6 py-4">{{ date_format(date_create($contrib['date']),"M j, Y") }}</td>
                    <td class="px-6 py-4 text-end">{{ number_format($contrib['amount'], 2) }}</td>
                        ...
                        <div class="action-menu relative">
                            ...
                            <div class="dropdown z-10 hidden absolute mt-2 right-0 text-base bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                                <ul class="py-2 text-start divide-y divide-gray-600">
                                    <li>
                                        <a href="{{ route('contribution.update', ['action' => 'approve', 'id' => $encryptedid_contrib]) }}" class="block px-4 py-2 text-md text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-100 dark:hover:text-white">Approve</a>
                                    </li>
                                    <li>
                                        <a href="{{ route('contribution.update', ['action' => 'decline', 'id' => $encryptedid_contrib]) }}" class="block px-4 py-2 text-md text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-red-500 dark:hover:text-white">Decline</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
